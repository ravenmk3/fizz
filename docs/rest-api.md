...
| mutexKey        | String          | 否  | MutexKey（互斥键），相同 key 串行执行          |
...
`progress` = `(succeededCount + failedCount + cancelledCount) * 100 / totalCount`，表示已到达终态的百分比。
...
- 作业不存在：返回 404
- 作业已经是终态（SUCCEEDED / FAILED / CANCELLED）：返回 409
