...
| 触发方式    | 场景         | 说明                                                |
| ----------- | ------------ | --------------------------------------------------- |
| 新作业提交  | JobService   | `scheduler.tell(JobSubmitted)` → 路由到 TenantJobScheduler |
| 任务完成    | TaskComponent | `parent.tell(TaskCompleted)` → 释放槽位，调度下一批 |
| 作业完成    | JobComponent | `parent.tell(JobCompleted)` → 释放槽位，激活下一个  |
| 作业取消    | JobService   | `scheduler.tell(CancelJob)` → 级联到 JobComponent   |
